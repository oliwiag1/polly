...
class SurveyCreate(BaseModel):
    ...
    questions: list[Question] = Field(
        ..., min_length=1, description="List of survey questions"
    )
...
class AnswerSubmit(BaseModel):
    ...
    respondent_id: str | None = Field(
        default=None, description="Optional respondent identifier"
    )
...
class QuestionStats(BaseModel):
    ...
    answer_distribution: dict[str, int] = Field(
        default_factory=dict, description="Distribution of answers"
    )

    # Typowa odpowiedź
    average_value: float | None = Field(
        default=None, description="Average value for numeric questions"
    )
...
class SurveyStats(BaseModel):
    ...
    last_response_at: datetime | None = Field(
        default=None, description="Timestamp of last response"
    )
