...
def log_execution(func: Callable[P, R]) -> Callable[P, R]:
    ...
    async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        try:
            ...
            logger.info(
                f"{func_name} completed successfully in {elapsed:.4f}s",
                module="decorators",
            )
            return result
            ...
            logger.exception(
                f"{func_name} failed after {elapsed:.4f}s with error: {e}",
                module="decorators",
            )
            raise
...
def handle_exceptions(func: Callable[P, R]) -> Callable[P, R]:
    ...
    async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        try:
            ...
            logger.warning(
                f"Validation error in {func.__name__}: {e}", module="decorators"
            )
            raise HTTPException(status_code=400, detail=str(e))
        except KeyError as e:
            # Wypisanie informacji o błędzie braku rekordu
            logger.warning(
                f"Not found error in {func.__name__}: {e}", module="decorators"
            )
            raise HTTPException(status_code=404, detail=f"Resource not found: {e}")
        except Exception as e:
            # Wypisanie informacji o ogólnym błędzie
            logger.exception(
                f"Unexpected error in {func.__name__}: {e}", module="decorators"
            )
