...
class TestSurveyServiceCreate:
    ...
    def test_create_survey_generates_links(self, survey_service, sample_survey_create, config):
    ...
    def test_create_survey_saves_to_database(self, survey_service, sample_survey_create, database):
...
class TestSurveyServiceSubmitResponse:
    ...
    def test_submit_response(self, survey_service, created_survey, sample_answer_submit):
        """Sprawdza wysyłanie odpowiedzi."""
        response = survey_service.submit_response(
            created_survey.id, 
            sample_answer_submit
        )
    ...
    def test_submit_response_saves_answers(self, survey_service, created_survey, sample_answer_submit):
    ...
    def test_submit_response_sets_timestamp(self, survey_service, created_survey, sample_answer_submit):
    ...
    def test_submit_response_survey_not_found(self, survey_service, sample_answer_submit):
        """Sprawdza błąd dla nieistniejącej ankiety."""
        with pytest.raises(ValueError, match="not found"):
...
class TestSurveyServiceValidation:
    ...
    def test_validate_single_choice_invalid_option(self, survey_service, created_survey):
...
class TestSurveyServiceStatistics:
    ...
    def test_get_statistics_rating_average(
        ...
        rating_stats = next(
            q for q in stats.questions_stats if q.question_id == "q4"
        )
    ...
    def test_get_statistics_distribution_text(
        self, survey_service, created_survey
    ):
        ...
        text_stats = next(
            q for q in stats.questions_stats if q.question_id == "q1"
        )
        
        assert text_stats.answer_distribution["Anna"] == 2
        assert text_stats.answer_distribution["Jan"] == 1
    
    def test_get_statistics_distribution_yes_no(
        self, survey_service, created_survey
    ):
        ...
        yes_no_stats = next(
            q for q in stats.questions_stats if q.question_id == "q5"
        )
    ...
    def test_get_statistics_distribution_multiple_choice(
        ...
        mc_stats = next(
            q for q in stats.questions_stats if q.question_id == "q3"
        )
